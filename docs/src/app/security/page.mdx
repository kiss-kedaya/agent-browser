...
Restrict which domains the browser can navigate to, preventing redirect-based attacks:
...
Supports exact match (`github.com`) and wildcard prefix (`*.example.com`). When navigation is blocked, the command returns an error.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>eval</code></td><td>eval, evalhandle, addscript, addinitscript</td></tr>
    ...
    <tr><td><code>get</code></td><td>get text/html/url/title, count, isvisible, etc.</td></tr>
